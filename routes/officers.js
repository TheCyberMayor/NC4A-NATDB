...
const officerValidation = [
    ...
    body('stateOfOrigin').notEmpty().withMessage('State of origin is required')
        .bail()
        .custom((value) => {
            // accept FCT and standard state names from the list
            return !!lgasByState[value];
        }).withMessage('Invalid state selected'),
    body('lga').notEmpty().withMessage('LGA is required')
        .bail()
        .custom((lga, { req }) => {
            const state = req.body.stateOfOrigin;
            if (!state || !lgasByState[state]) return true; // other validations will catch missing state
            return lgasByState[state].includes(lga);
    ...
    body('facebook').notEmpty().withMessage('Facebook handle is required'),
    body('twitter').optional().isString(),
    body('instagram').optional().isString(),
    body('tiktok').notEmpty().withMessage('TikTok handle is required'),
